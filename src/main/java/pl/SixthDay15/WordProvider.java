...
public class WordProvider {
    ...
    public static void streamWordsWithD(List<String> words){
        List<String> resultListOne =  words.stream()
                .filter(word -> word.startsWith("d"))
                .collect(Collectors.toList());
        System.out.println(resultListOne);
    }

    public static void streamWordsWithAAndFourLetters(List<String> words){
        List<String> resultListTwo =  words.stream()
                .filter(word -> word.startsWith("a") && word.length() <= 4)
                .collect(Collectors.toList());
        System.out.println(resultListTwo);
    }

    public static void changeFirstLetter(List<String> words){
        List<String> resultList =  words.stream()
                .map(word -> word.replaceFirst(word.substring(0, 1), word.substring(0, 1).toUpperCase()))
                .collect(Collectors.toList());
        System.out.println(resultList);
    }
    public static void changeFirstLetterSecondTask(List<String> words){
        List<String> resultList =  words.stream()
